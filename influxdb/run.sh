...
then
  ...
  influx -host=localhost -port=${INFLUXDB_PORT} -username=${INFLUXDB_ADMIN_USER} -password="${INFLUXDB_ADMIN_PASSWORD}" -execute="CREATE DATABASE ${INFLUXDB_DATABASE} WITH DURATION ${INFLUXDB_DATABASE_DURATION}"
